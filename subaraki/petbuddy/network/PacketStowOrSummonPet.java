			...
			((WorldServer) ctx.getServerHandler().playerEntity.world).addScheduledTask(() -> {
				...
				if (petid == null)
					if (inventory.hasCooldown()) {
						...
						player_mp.sendMessage(new TextComponentString(TextFormatting.RED + inventory.getPetName()
								+ " is still trying to resurrect ... ~" + time[0] + ":" + time[1]));
					} else {
						StowOrSummonLogic.givePet(player_mp);
						player_mp.sendMessage(
								new TextComponentString(TextFormatting.BLUE + "Summoned " + inventory.getPetName()));
					}
				else
				// stow pet
				if (world.getEntityByID(petid) != null && world.getEntityByID(petid) instanceof EntityPetBuddy) {
					...
					player_mp.sendMessage(
							new TextComponentString(TextFormatting.GREEN + "Stowed " + inventory.getPetName()));
				} else {
					StowOrSummonLogic.givePet(player_mp);
					player_mp.sendMessage(
							new TextComponentString(TextFormatting.BLUE + "Summoned " + inventory.getPetName()));
